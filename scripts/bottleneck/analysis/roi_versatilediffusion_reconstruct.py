...
pred_text = np.load(f'data/predicted_features/subj01/nsd_cliptext_{cap_length}_roi_nsdgeneral.npy')
pred_text = torch.tensor(pred_text).half().cuda(1)

pred_vision = np.load(f'data/predicted_features/subj01/nsd_clipvision_{cap_length}_roi_nsdgeneral.npy')
...
res_dir = f'results/versatile_diffusion_from_{cap_length}_captions/subj01/roi/'
...
for im_id in range(len(pred_vision)):
    ...
    x[0].save(f'results/versatile_diffusion_from_{cap_length}_captions/subj01/roi/{im_id}.png')
